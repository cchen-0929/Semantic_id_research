...
from data.recdata import RecDataItemData
from data.recdata import RecDataSeqData
...
class ItemData(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        if dataset == RecDataset.RECDATA:
            recdata = RecDataItemData(
                root=root,
                split=kwargs.get("split", "Beauty"),
                train_test_split=train_test_split,
            )
            self.item_data = recdata.item_data
            self.item_text = recdata.item_text
            return
...
class SeqData(Dataset):
    ...
    def max_seq_len(self):
        if hasattr(self, "_recdata"):
            return self._recdata.max_seq_len
        return self._max_seq_len

    def __len__(self):
        if hasattr(self, "_recdata"):
            return len(self._recdata)
        return self.sequence_data["userId"].shape[0]
  
    def __getitem__(self, idx):
        if hasattr(self, "_recdata"):
            return self._recdata[idx]
