...
def infer_user_item_num(cfg):
    dataset_cfg = cfg.datasets_cfg
    if "recdata_ood_sasrec" in dataset_cfg:
        parsed = load_recdata(dataset_cfg.recdata_ood_sasrec.path)
        return parsed.n_user, parsed.m_item
    if "recdata_ood" in dataset_cfg:
        parsed = load_recdata(dataset_cfg.recdata_ood.path)
        return parsed.n_user, parsed.m_item

    if "movie_ood_sasrec" in dataset_cfg:
        data_dir = dataset_cfg.movie_ood_sasrec.path
    elif "amazon_ood_sasrec" in dataset_cfg:
        data_dir = dataset_cfg.amazon_ood_sasrec.path
    else:
...
def main():
    ...
    user_num, item_num = infer_user_item_num(cfg)
