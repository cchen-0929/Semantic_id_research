...
from minigpt4.datasets.datasets.recdata_datasets import RecDataOOData, RecDataOOData_sasrec
...
@registry.register_builder("recdata_ood")
class RecDataOODBuilder(RecBaseDatasetBuilder):
    train_dataset_cls = RecDataOOData
    ...
    def build_datasets(self, evaluate_only=False):
        ...
        datasets["train"] = dataset_cls(
            text_processor=self.text_processors["train"],
            ann_paths=[f"{storage_path}=train"],
        )
        datasets["valid"] = dataset_cls(
            text_processor=self.text_processors["train"],
            ann_paths=[f"{storage_path}=valid"],
        )
        datasets["test"] = dataset_cls(
            text_processor=self.text_processors["train"],
            ann_paths=[f"{storage_path}=test"],
        )
        return datasets


@registry.register_builder("recdata_ood_sasrec")
class RecDataOODBuilder_sasrec(RecBaseDatasetBuilder):
    train_dataset_cls = RecDataOOData_sasrec
