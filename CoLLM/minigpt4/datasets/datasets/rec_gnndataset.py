...
class RecDataGnnDataset(BasicDataset):
    """
    Build LightGCN graph directly from RecData txt files.
    """

    def __init__(self, config, path):
        print("loading RecData graph from:", path)
        self.split = config.A_split
        self.folds = config.A_n_fold
        ...
        with open(train_file, "r") as f:
            for line in f:
                ...
                for item in train_items:
                    rows.append([user, item, 1.0])

        if len(rows) == 0:
            raise RuntimeError(f"No positive train interactions found in {train_file}")

        self.train = pd.DataFrame(rows, columns=["user", "item", "label"])
        self.m_users = max_user + 1
        self.n_items = max_item + 1
        self.train_size = len(rows)
        self.Graph = None

        print(f"{self.train_size} interactions for graph building")
        print(f"{self.m_users} users, {self.n_items} items")

        self._register_graph()
        print(f"{dataset} RecData graph is ready")
    ...
    def _split_A_hat(self, A):
        ...
        for i_fold in range(self.folds):
            ...
            fold_tensor = self._convert_sp_mat_to_sp_tensor(A[start:end]).coalesce()
            if self._use_cuda:
                fold_tensor = fold_tensor.cuda()
            A_fold.append(fold_tensor)
        return A_fold
    ...
    def _register_graph(self):
        self.getSparseGraph_mode_a2("lightgcn_recdata")
    ...
    def getSparseGraph_mode_a2(self, mode):
        ...
        print("loading adjacency matrix")
        if self.Graph is None:
            cache_file = self.path + "/s_pre_adj_mat_" + mode + ".npz"
            try:
                ...
                adj_mat = sp.csr_matrix(
                    (pos[:, 2], (pos[:, 0], pos[:, 1])),
                    shape=(self.m_users + self.n_items, self.m_users + self.n_items),
                )
                adj_mat = adj_mat.todok()
                rowsum = np.array(adj_mat.sum(axis=1))
                d_inv = np.power(rowsum, -0.5).flatten()
